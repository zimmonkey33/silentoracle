...
import { Sparkles, AlertCircle, User2, Building2, Trophy, BookOpen, Globe, Eye, Calendar, Heart, Database, Hash, MessageCircle } from "lucide-react";
...
export default function Home() {
  ...
  // Load saved birthdate on mount
  useEffect(() => {
    try {
      const bd = localStorage.getItem("so_bd");
      if (bd) {
        setBirthDate(bd);
        setProfile(buildProfile(bd));
      }
    } catch {
      /* ignore */
    }
  ...
  async function handlePerson(v: PersonFormValues) {
    await callAnalyze({
      ...
      partnerBirthDate: v.partnerBirthDate || null,
    });
  }
  ...
  async function handleSports(v: SportsFormValues) {
    await callAnalyze({
      ...
      team1Name: v.team1Name,
      team1City: v.team1City || null,
      team1FoundingYear: v.team1FoundingYear ? parseInt(v.team1FoundingYear, 10) : undefined,
      team1Jerseys: v.team1Jerseys.split(/[\s,]+/).map((x) => parseInt(x, 10)).filter((x) => Number.isFinite(x) && x > 0 && x < 100),
      ...
      team2Name: v.team2Name,
      team2City: v.team2City || null,
      team2FoundingYear: v.team2FoundingYear ? parseInt(v.team2FoundingYear, 10) : undefined,
      team2Jerseys: v.team2Jerseys.split(/[\s,]+/).map((x) => parseInt(x, 10)).filter((x) => Number.isFinite(x) && x > 0 && x < 100),
  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ background: `${T.bg2}f0`, borderColor: T.border }}
      >
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            <div
              className="grid size-10 place-items-center rounded-xl shadow-md"
              style={{ background: `linear-gradient(135deg, ${T.orange}, ${T.orangeHi})`, color: "#000" }}
            >
          ...
          {/* Mode switcher */}
          <div
            className="flex rounded-md p-1 gap-1"
          ...
          >
            <button
              ...
              style={{
                background: mode === "oracle" ? `${T.orange}22` : "transparent",
                color: mode === "oracle" ? T.orange : T.textDim,
                border: `1px solid ${mode === "oracle" ? T.orange : "transparent"}`,
                letterSpacing: "2px",
              }}
            >
              🔮 ORACLE
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs font-bold rounded transition-colors"
              style={{
                background: mode === "analyzer" ? `${T.orange}22` : "transparent",
                color: mode === "analyzer" ? T.orange : T.textDim,
                border: `1px solid ${mode === "analyzer" ? T.orange : "transparent"}`,
                letterSpacing: "2px",
              }}
            >
              📊 ANALYZER
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pt-8 pb-6 text-center">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-balance text-sm sm:text-base" style={{ color: T.textMid }}>
          {mode === "oracle" ? (
            <>
              Build your <strong style={{ color: T.orange }}>personal numerology profile</strong>, track{" "}
              <strong style={{ color: T.orange }}>daily energy</strong>, check{" "}
              <strong style={{ color: T.orange }}>compatibility</strong>, browse{" "}
              <strong style={{ color: T.orange }}>1,400+ entities</strong>, and ask the{" "}
              <strong style={{ color: T.orange }}>Silent Oracle AI</strong> for guidance.
            </>
          ) : (
            <>
      ...
      {mode === "oracle" && (
        <section className="mx-auto w-full max-w-3xl px-4 pb-16 flex-1">
          <Tabs value={oracleTab} onValueChange={setOracleTab}>
            <TabsList
              className="mb-5 flex h-auto w-full flex-wrap gap-1 p-1 overflow-x-auto oracle-scrollbar"
              ...
              {ORACLE_TABS.map((t) => {
                ...
                return (
                  <TabsTrigger
                    key={t.value}
                    value={t.value}
                    className="flex-1 min-w-[100px] flex items-center gap-1.5 px-2 py-2 text-[10px] font-bold tracking-wider data-[state=active]:text-[#FF7A00]"
                    style={{
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: active ? `${T.orange}11` : T.card,
                      boxShadow: active ? `0 4px 12px ${T.orange}33` : "none",
                    }}
                  >
                    <Icon className="size-5" style={{ color: active ? T.orange : T.textDim }} />
                    <p className="mt-2 font-semibold" style={{ color: T.text }}>
                      {opt.label}
                    </p>
              ...
              {data.entityType === "person" && data.report && (
                <Tabs defaultValue="overview" className="w-full">
                  <TabsList
                    ...
                    style={{ background: T.card, border: `1px solid ${T.border}` }}
                  >
                    {["overview", "numerology", "chinese", "personality", "career", "relationships", "compatibility", "health", "forecast", "lucky"]
                      .filter((v) => v !== "compatibility" || data.compatibility)
                      .map((v) => (
                        <TabsTrigger
                          key={v}
                          value={v}
                          className="flex-1 min-w-[90px] capitalize text-[11px] font-bold tracking-wider"
                          style={{ color: T.textMid, letterSpacing: "1.5px" }}
                        >
                          {v}
                        </TabsTrigger>
                      ))}
                  ...
                  <TabsContent value="chinese" className="mt-0">
                    {data.chineseYearSign ? (
                      <ChineseYearSignCard sign={data.chineseYearSign} contextLabel="Your birth year" />
                    ) : (
                      <p className="text-sm" style={{ color: T.textDim }}>Chinese zodiac data not available.</p>
                    )}
                  </TabsContent>
                  <TabsContent value="personality" className="mt-0"><PersonalitySection data={data} /></TabsContent>
                  <TabsContent value="career" className="mt-0">
                    <div className="grid gap-5"><CareerSection data={data} /><RoadmapSection data={data} /></div>
                  </TabsContent>
                  ...
                  <TabsContent value="forecast" className="mt-0">
                    <div className="grid gap-5"><ForecastSection data={data} /><RoadmapSection data={data} /></div>
                  </TabsContent>
                ...
                <div className="grid gap-5">
                  ...
                  {data.chineseYearSign && (
                    <ChineseYearSignCard
                      ...
                      contextLabel={data.entityType === "country" ? "Country's founding/independence year" : "Company's founding year"}
                    />
                  )}
                </div>
              )}
      ...
      >
        <div className="mx-auto max-w-7xl px-4 py-6 text-center text-xs" style={{ color: T.textDim }}>
          <p className="mb-1">
            ...
            z-ai-web-dev-sdk — no external API keys required.
          </p>
          <p>For reflection and entertainment. All calculations run server-side; nothing is stored.</p>
        </div>
      </footer>
    </main>
  );
