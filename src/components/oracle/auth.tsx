...
export async function activatePro(plan: "monthly" | "yearly" = "monthly"): Promise<{ ok: boolean; checkoutUrl?: string; demo?: boolean; error?: string }> { const res = await fetch("/api/auth/subscribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plan }) }); const data = await res.json(); if (!res.ok) return { ok: false, error: data.error }; return data; }
